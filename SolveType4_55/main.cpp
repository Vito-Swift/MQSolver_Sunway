...
int main() {
    ...
    FILE *fr = fopen("mq-resident4-55-1.txt", "rb");
    FILE *frr = fopen("mq4-55-1-f.txt", "rb");
    poly fullpoly[M][N + 1];
    ...
    // Key Boundary Init
    uint64_t search_size = 0x10624dd2f1;
    uint64_t init_key = search_size * rank;
    uint64_t end_key = search_size * (rank + 1);
    ...
    for (int64_t key = init_key; key < end_key; key++) { // Exhaustive Search
        ...
        std::cout << std::bitset<19>(mask) << std::endl;
        if (!(mask & clist[9])) {
            uint32_t sol[9] = {0};
