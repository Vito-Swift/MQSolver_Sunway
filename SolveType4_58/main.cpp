...
// gf2 arithmetic
void checkConsist_22x9(uint32_t clist[10], uint32_t &mask);

void extractSolution_22x9(const uint32_t clist[10], uint32_t sol[9]);
...
int main() {
    ...
    FILE *fr = fopen("mq-resident4-58-2.txt", "rb");
    FILE *frr = fopen("mq4-58-2-f.txt", "rb");
    poly fullpoly[M][N + 1];
    ...
    int los = 0;
    uint64_t guessMask = 0x1ffffffffffff;

    uint64_t segSize = 0xd1b71758e;
    ...
    for (int64_t key = init_key; key < end_key; key++) { // Exhaustive Search
        ...
        checkConsist_22x9(clist, mask);
        if (!(mask & clist[9])) {
            uint32_t sol[9] = {0};
            extractSolution_22x9(clist, sol);
            uint32_t guess[N] = {0};
